...
def save_encrypted_cookie(encrypted_cookie_data, password=None, debug_info=None):
    """
    ...
    Args:
        ...
        debug_info (dict, optional): 调试信息，包含请求详情
    ...
    try:
        # 验证输入数据
        if not encrypted_cookie_data:
            raise ValueError("加密的cookie数据为空")

        if not isinstance(encrypted_cookie_data, str):
            raise ValueError(f"加密的cookie数据类型错误: {type(encrypted_cookie_data)}")

        # 检查数据格式
        if len(encrypted_cookie_data) < 10:
            raise ValueError(f"加密的cookie数据太短: {len(encrypted_cookie_data)} 字符")
        ...
        if debug_info:
            ...
            debug_file = f"{debug_dir}/cookie_debug_{timestamp}.txt"

            with open(debug_file, 'w', encoding='utf-8') as f:
                f.write(f"=== Cookie解密调试信息 ===\n")
                f.write(f"时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"时间戳: {timestamp}\n")
                f.write(f"加密数据长度: {len(encrypted_cookie_data)}\n")
                f.write(f"加密数据预览: {encrypted_cookie_data[:100]}...\n")
        ...
        if not cookie_content:
            raise ValueError("解密后的cookie内容为空")
        ...
        # 记录详细的错误信息
        error_msg = f"保存加密cookie失败: {str(e)}"
        # 即使在错误情况下也保存调试信息
        if debug_info:
            import json
            import time
            ...
            debug_dir = "/tmp/cookie_debug"
            os.makedirs(debug_dir, exist_ok=True)
            error_debug_file = f"{debug_dir}/cookie_error_{timestamp}.txt"

            with open(error_debug_file, 'w', encoding='utf-8') as f:
                f.write(f"=== Cookie解密错误信息 ===\n")
                f.write(f"时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"时间戳: {timestamp}\n")
                f.write(f"错误信息: {str(e)}\n")
                f.write(f"加密数据长度: {len(encrypted_cookie_data) if encrypted_cookie_data else 0}\n")
                f.write(f"加密数据预览: {encrypted_cookie_data[:100] if encrypted_cookie_data else ''}...\n")
                f.write(f"调试信息:\n{json.dumps(debug_info, indent=2, ensure_ascii=False)}\n")
                f.write("=" * 50 + "\n")

            error_msg += f" | 错误调试信息已保存到: {error_debug_file}"
